...
ALY.ServiceInterface.Query = {
  buildRequest: function buildRequest(req) {
    ...
    httpRequest.params = {
      ...
      timestamp: ALY.util.date.topDateFormat(ALY.util.date.getDate()),
      format: 'json',
      app_key: '',
      v: '2.0',
      sign_method: 'hmac'
    ...
    httpRequest.body = ALY.util.queryParamsToString(httpRequest.params);
  },
  ...
  extractError: function extractError(resp) {
    ...
    if (body.match('<UnknownOperationException')) {
      ...
      data = new ALY.XML.Parser({}).parse(body);
    }
  ...
  extractData: function extractData(resp) {
    console.log('success', resp.httpResponse.body.toString());
    return;
    ...
    var parser = new ALY.XML.Parser(rules);
    var data = parser.parse(resp.httpResponse.body.toString());

    if (req.service.api.resultWrapped) {
      if (data[wrapperKey]) {
        ALY.util.update(data, data[wrapperKey]);
