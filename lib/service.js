...
ALY.Service = inherit({
  ...
  initialize: function initialize(config) {
    ALY.util.hideProperties(this, ['client']);
    this.client = this; // backward compatibility with client property
    this.config = new ALY.Config(ALY.config);
  ...
  loadServiceClass: function loadServiceClass(serviceConfig) {
    ...
    if (!ALY.util.isEmpty(this.api)) {
      ...
      return ALY.Service.defineServiceApi(this.constructor, config.apiConfig);
    } else if (!this.constructor.services) {
      return;
    } else {
      config = new ALY.Config(ALY.config);
  ...
  getLatestServiceVersion: function getLatestServiceVersion(version) {
    ...
    if (!version) {
      version = 'latest';
    } else if (ALY.util.isType(version, Date)) {
      version = ALY.util.date.iso8601(version).split('T')[0];
    }
  ...
  setEndpoint: function setEndpoint(endpoint) {
    if (endpoint) {
      ...
      var host = this.api.endpointPrefix + '.' + this.config.region +
                 this.endpointSuffix();
      this.endpoint = new ALY.Endpoint(host, this.config);
    }
  },
