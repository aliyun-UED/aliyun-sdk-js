...
function Shape(rules, options) {
  ...
  ALY.util.each.call(this, rules, function (key, value) {
...
Shape.prototype = {
  ...
  set_members: function(members) {
    ...
    if (type === 'structure') {
      this.rules.members = {};
      ALY.util.each.call(this, members, function(memberName, memberRules) {
...
InputShape.prototype = ALY.util.merge(Shape.prototype, {
...
OutputShape.prototype = ALY.util.merge(Shape.prototype, {
...
function Operation(rules, options) {
  ...
  function normalizeInputs() {
    ...
    ALY.util.each(rules.input.members, function (name, rule) {
      if (rule.payload) {
        hasPayload = true;
        payload = name;
        delete rule.payload;
    ...
    if (wrapper) rules.input = ALY.util.merge({wrapper: wrapper}, rules.input);
    if (payload) rules.input = ALY.util.merge({payload: payload}, rules.input);
  }

  function normalizeOutputs() {
    var moveUp = null;

    ALY.util.each(rules.output.members, function(memberName, rule) {
    ...
    if (moveUp) {
      ...
      ALY.util.update(rules.output.members, rule.members);
    }
  }

  rules = ALY.util.copy(rules);
...
function Translator(api, options) {
  ...
  function setOperations() {
    ...
    ALY.util.each(api.operations, function (key, value) {
  ...
  function setPagination() {
    if (api.pagination) {
      ...
      ALY.util.each(api.pagination, function (key, value) {
        var object = {};
        ALY.util.each(value, function (k2, v2) { object[inflect(k2)] = v2; });
        translate.pagination[key[0].toLowerCase() + key.substr(1)] = object;
      });
    }
  }

  if (typeof api === 'string' || ALY.util.Buffer.isBuffer(api)) {
    api = JSON.parse(api);
  }
