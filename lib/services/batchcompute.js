...
ALY.BatchCompute = ALY.Service.defineService('batchcompute', ['2015-06-30'], {
    ...
    extractData: function extractData(resp, operation) {
        ...
        switch (operation) {
            case 'listJobStatus':
                resp.data.data = this.getFormatters().formatJobStatusList(result);
                break;
            case 'getJobStatus':
                resp.data.data = this.getFormatters().formatJobStatus(result);
                break;
            case 'getJob':
                resp.data.data = this.getFormatters().formatJobDescription(result);
                break;
            case 'listTaskStatus':
                resp.data.data = this.getFormatters().formatTaskStatusList(result);
                break;
            case 'listImages':
    ...
    getFormatters: function () {
        ...
        return {
            ...
            formatJobStatusList: function (data) {
                var that = this;
                var t = [];
                Object.keys(data).forEach(function (k) {
                    t.push(that.formatJobStatus(data[k]));
                });
            ...
            formatTaskStatusList: function (data) {
                /*{ CountTask:
                 { EndTime: 1435520792,
