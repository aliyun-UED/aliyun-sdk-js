...
ALY.OSS = ALY.Service.defineService('oss', ['2013-10-15'], {
  ...
  getSignedUrl: function getSignedUrl(operation, params, callback) {
    ...
    function signedUrlBuilder() {
      ...
      var securityToken = request.httpRequest.headers['x-oss-security-token'];
      if(securityToken){ 
        delete request.httpRequest.headers['x-oss-security-token'];

        request.httpRequest.path += (request.httpRequest.path.indexOf('?')==-1)
